...
- **Queue Start Number**
  Номер очереди `netfilter` для перехвата пакетов, диапазон `0–65535`.
...
B4 предоставляет интерактивную документацию REST API через Swagger UI. Для включения добавьте в конфигурационный файл `b4.json`:
...
После перезапуска сервиса документация доступна по адресу:

```text
http://IP-вашего-устройства:7000/swagger/
```

Swagger UI позволяет просматривать все доступные API-эндпоинты, их параметры и тестировать запросы прямо из браузера.

:::tip Авторизация в Swagger
Если включена авторизация веб-интерфейса, сначала выполните `POST /api/auth/login` для получения токена, затем нажмите кнопку **Authorize** и введите `Bearer {ваш_токен}`.
:::

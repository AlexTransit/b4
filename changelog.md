...
- ADDED: **QUIC Initial faking strategy** - new "QUIC Initial (auto)" option in the UDP Faking Strategy dropdown. b4 generates a fresh randomized QUIC Initial packet for every fake, with random connection IDs each time, so no upload is needed and the bytes can't be fingerprinted by repetition. Recommended packet size for this mode is 1200 bytes.
